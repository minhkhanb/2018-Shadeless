...
void TopeBox_PromoGame::RenderGamePromo(float x, float y)
{
	...
	if (typeAds == enumTypeTopeboxAds::TOPEBOX_ADS_ICON)
	{
		...
		G()->ClearScale();
		GET_SPRITE(SPRITEID_FONT_NAME_FACEBOOK)->_char_spacing = 2;
		GET_SPRITE(SPRITEID_FONT_NAME_FACEBOOK)->DrawAnsiTextFormat(G(), _renderx + 55 + _icon_w, _rendery + _icon_h * 1.3f + 25, _RIGHT | _VCENTER, "%s", _des);
	}
...
void TopeBox_PromoGame::SetTopeBoxPromoGameInforFromConfig() {
	...
	if (Device::RemoteConfigGetInteger("GamePromoteVersion") != NULL)
	{
		...
		game->revison = game->_user._game_promo_revision;
	}
#endif
	PDEBUG("\n revison:%d -- game->_user._game_promo_revision:%d", game->revison, game->_user._game_promo_revision);
	PDEBUG("\n _link_icon:%s -- game->_user._game_promo_icon_id:%s -- strcmp(game->_user._game_promo_icon_id, _link_icon):%d", _link_icon, game->_user._game_promo_icon_id, strcmp(game->_user._game_promo_icon_id, _link_icon));
	if (game->revison != game->_user._game_promo_revision && game->revison >= 0)
	{
		PDEBUG("\n ##### update GameInfo: %d", game->revison);
		game->_user._game_promo_revision = game->revison;
