...
const char* Pine_Date_Time::GetStringTime(INT_TIME milisecond, bool useDot)
{
	...
	if (time.getDay() > 0 )
	{
		if (useDot)
		{
			sprintf(_timeString, "%dd %02d:%02d:%02d", time.getDay(), time.getHour(), time.getMinute(), time.getSecond());
		}
		else
		{
...
const char* Pine_Date_Time::GetStringTimeFormat(INT_TIME milisecond, bool useDot)
{
	...
	if (useDot)
	{
		sprintf(_timeString, "%02d:%02d:%02d", time.GetTotalHour(), time.getMinute(), time.getSecond());
	}
	else
	{
